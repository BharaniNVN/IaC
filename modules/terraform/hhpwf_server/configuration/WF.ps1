Configuration WF {
    ...
    Import-DscResource -ModuleName xCertificatePermission
    ...
    [System.String[]] $certificateThumbprints = (Get-ChildItem -Path Cert:\LocalMachine\My).Where({ $_.Subject -match $($CertificateSubjects -join '|') }).Thumbprint
    ...
    Node $NodeName {
        ...
        'NET-Framework-Core', 'NET-HTTP-Activation', 'NET-Framework-45-Core', `
            'NET-Framework-45-ASPNET', 'NET-WCF-HTTP-Activation45', 'NET-WCF-TCP-PortSharing45', `
            'WAS-Process-Model', 'WAS-NET-Environment', 'WAS-Config-APIs' | ForEach-Object {
        ...
        $dotNetCore207Package = @(
            @{Name = 'Microsoft .NET Core Runtime - 2.0.7 (x64)'; Url = 'https://proddscstg.blob.core.windows.net/software/.NET%20Core%202.0.8/dotnet-runtime-2.0.7-win-x64.msi'; Id = '59F7377C-012A-4D30-A658-3C3B97EBEBE5' },
            @{Name = 'Microsoft .NET Core Host - 2.0.7 (x64)'; Url = 'https://proddscstg.blob.core.windows.net/software/.NET%20Core%202.0.8/dotnet-host-2.0.7-win-x64.msi'; Id = '44A0581F-4D12-4547-9D73-6239C4E24B07' },
            @{Name = 'Microsoft ASP.NET Core 2.0.8 Runtime Package Store (x64)'; Url = 'https://proddscstg.blob.core.windows.net/software/.NET%20Core%202.0.8/AspNetCore.2.0.8.RuntimePackageStore_x64.msi'; Id = '93571190-2F86-30D7-A78E-359FA888E54B' },
            @{Name = 'Microsoft .NET Core Host FX Resolver - 2.0.7 (x64)'; Url = 'https://proddscstg.blob.core.windows.net/software/.NET%20Core%202.0.8/dotnet-hostfxr-2.0.7-win-x64.msi'; Id = '2BB77856-A486-4C37-B9F3-3FA12838880C' }
        )

        foreach ($app in $dotNetCore207Package) {
            ...
            $dependsOnNetCore207 += @("[xPackage]$($app.Name -replace('[ ()]', ''))")
        }
        ...
        'Web-Default-Doc', 'Web-Dir-Browsing', 'Web-Http-Errors', 'Web-Static-Content', 'Web-Http-Redirect', `
            ...
            'Web-Dyn-Compression', 'Web-Filtering', 'Web-IP-Security', 'Web-Net-Ext', 'Web-Net-Ext45', 'Web-Asp-Net45', `
        ...
        $certificateThumbprints | ForEach-Object {

            xCertificatePermission $_ {
                Ensure      = 'Present'
                Location    = 'LocalMachine'
                Thumbprint  = $_
                Store       = 'My'
                UserAccount = $AppPoolAccount
                Permission  = 'Read'
                DependsOn   = '[Computer]JoinDomain'
            }
        }
        ...
        foreach ($app in $NetCore313Package) {
            ...
            xPackage $app.Name.Replace(' ', '') {
                ...
                Path      = $app.Url
                ProductId = $app.Id
                DependsOn = @($dependsOnFeatures, $dependsOnNetCore207, '[xWebSite]DefaultWebSite')
            }
        }
        ...
        $apps = @(
            ...
            #@{Name = 'Microsoft Web Platform Installer 5.1'; Url = 'https://proddscstg.blob.core.windows.net/software/WebPlatformInstaller/WebPlatformInstaller_x64_en-US_5.1_5.1.51515.0.msi' },
            ...
            #@{Name = 'IIS URL Rewrite Module 2'; Url = 'C:\Program Files\Microsoft\Web Platform Installer\WebpiCmd-x64.exe'; Args = '/install /products:UrlRewrite2 /AcceptEula'; Deps = '[xPackage]MicrosoftWebPlatformInstaller5.1' }
            @{Name = 'IIS URL Rewrite Module 2'; Url  = 'https://proddscstg.blob.core.windows.net/software/rewrite_amd64_en-US.msi'; Args = '/quiet' }
        )
        ...
        $eventLogs = @(
            @{LogName = 'AuthAPI'; Source = 'HomeHealth.AuthAPI'; Force = $true }
