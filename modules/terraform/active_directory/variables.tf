...
variable "domain_admin" {
  description = "Account with Domain Administrator privilege level to promote a new Domain Controller to the domain."
}

variable "domain_password" {
  description = "Password for the account with Domain Administrator privilege level."
}
...
variable "deploy_domain" {
  description = "Whether to deploy a new domain with primary and secondary Domain Controllers or join existing domain with secondary DCs."
  default     = false
}

variable "ad_ou" {
  description = "List of objects which describes necessary Organizational Unit structure to create: path - distinguished name to the parent object where OU should be created and name - list of OU names to create."
  default     = []
  type        = list(object({ path = string, name = list(string) }))
}

variable "ad_site" {
  description = "Active Directory Site name where newly promoted domain controllers should be placed."
  default     = "Default-First-Site-Name"
}

variable "dns_forwarders" {
  description = "List of recursive DNS servers which should be configured as forwarders."
  default     = ["1.1.1.1", "8.8.8.8", "168.63.129.16"]
  type        = list(string)
}

variable "forward_dns_zone_names" {
  description = "List of the forward lookup zone names."
  default     = []
  type        = list(string)
}

variable "reverse_dns_zone_names" {
  description = "List of the reverse lookup zone names without in-addr.arpa suffix."
  default     = []
  type        = list(string)
}

variable "dns_records" {
  description = "List of the 'A' DNS records which should be created in the domain zone."
  default     = []
  type        = list(object({ name = string, zone = string, ip = string }))
}
