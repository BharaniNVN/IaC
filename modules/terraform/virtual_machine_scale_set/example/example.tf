  ...
  extension = [
    {
      ...
      "settings" = jsonencode({
        "commandToExecute" = "powershell -Command \"& { Set-ExecutionPolicy Bypass -Scope Process -Force; iex ((New-Object System.Net.WebClient).DownloadString('https://chocolatey.org/install.ps1')); choco install googlechrome -y; choco install chromedriver -y; }\""
      })
    }
  ]
}
