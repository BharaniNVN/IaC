...
variable "enable_public_loadbalancer" {
  description = "Whether to deploy or not public Azure Load balancer."
  default     = false
}
...
variable "folders" {
  description = "List of the folders to create in the root of OS drive."
  default     = []
  type        = list(string)
}
...
variable "sql_port" {
  description = "TCP port that SQL Server should be listening on."
  default     = 1433
}

variable "sftp_admin_account" {
  description = "Name of the SFTP local administrator account which will be created during install."
}

variable "sftp_admin_password" {
...
variable "azure_firewall_resource" {
  description = "Azure Firewall resource on which required ports will be opened."
  default     = null
  type        = object({ name = string, resource_group_name = string })
}

variable "azure_firewall_public_ip_address" {
  description = "List of Azure Firewall public IP addresses for which NAT rules should be mapped. Count of IP addresses should match the count of target translated addresses."
  default     = []
  type        = list(string)
}

variable "rule_collection_priority" {
  description = "Priority of rule collection. Possible values are between 100 - 65000."
  default     = 500
}
