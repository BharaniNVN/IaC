locals {
  ...
  # virtual_machine_object   = [for k, v in local.virtual_machine : { "name" : k, "id" : v }] # used by Hybrid Worker module
  virtual_machine_resource = { for n in module.virtual_machine.name : n => module.virtual_machine.id[n] }
  pipelines_agent_svc_name = format("vstsagent.%s.%s", var.azure_devops_account, coalesce(var.azure_devops_deployment_pool, format("%s-%s", var.azure_devops_project, var.azure_devops_deployment_group)))
...
resource "azurerm_virtual_machine_extension" "dsc" {
  ...
  settings = <<SETTINGS
    {
      ...
      "configurationArguments": {
        ...
        "SQL_ISO": "${var.sql_iso_path}",
        "SSMS_ISO": "${var.ssms_install_path}",
        "SQLport": "${var.sql_port}",
        "SQLlogins": ${jsonencode(var.sql_logins)},
        "FromAddress": "${var.from_address}",
        "ReplyToAddress": "${var.replyto_address}",
        "SMTPport": "${var.smtp_port}",
        "SMTPserver": "${var.smtp_server}",
        "SQLAdminAccounts": ${jsonencode(var.sql_admin_accounts)},
        "SQLMemoryMin": "${var.sql_memory_min}",
        "SQLMemoryMax": "${var.sql_memory_max}",
        "SQLInstanceName": "${var.sql_instance_name}",
        "MyAnalyticsPack": ${var.install_myanalytics_software_pack},
        "IrKey": "${var.integration_runtime_key}",
        "DeploymentAgentServiceName": "${var.azure_devops_extension_version == null ? "" : format("%s.%s", local.pipelines_agent_svc_name, each.value)}",
        "TimeZone": "${var.timezone == null ? "" : var.timezone}",
        "NPMDPort": "${var.npmd_port}",
        "nxlog_conf": "${var.nxlog_conf}",
        "nxlog_pem": "${var.nxlog_pem}",
        "OracleSQLDeveloper": ${var.install_oracle_sql_developer},
        "OracleClient": ${var.install_oracle_client},
        "OracleClientArchive": "${var.oracle_client_archive}",
        "OracleClientResponseFile": "${var.oracle_client_response_file}",
        "OnPremisesDataGateway": ${var.install_on_premises_data_gateway},
        "OnPremisesDataGatewayPSMVersion": "${var.on_premises_data_gateway_psm_version}",
        "OnPremisesDataGatewayTenant": "${data.azurerm_client_config.current.tenant_id}"
      }
    }
  SETTINGS
  ...
  protected_settings = <<PROTECTEDSETTINGS
    {
      ...
      "configurationArguments": {
        ...
        "SMTPAccountCredential": {
          ...
          "Password": "${var.smtp_pswd}"
        },
        "SqlServiceCredential": {
          "UserName": "${var.sql_service_user}",
          "Password": "${var.sql_service_pass}"
        },
        "SqlAgentServiceCredential": {
          "UserName": "${var.sql_agent_user}",
          "Password": "${var.sql_agent_pass}"
        },
        "SqlSACredential": {
        ...
        "DeploymentAgentCredential": {
          "UserName": "${var.deployment_agent_account}",
          "Password": "${var.deployment_agent_password}"
        },
        "OnPremisesDataGatewayCredential": {
          "UserName": "${var.on_premises_data_gateway_ad_application_id}",
          "Password": "${var.on_premises_data_gateway_ad_application_secret}"
        }
      }
    }
  PROTECTEDSETTINGS
  ...
  depends_on = [azurerm_virtual_machine_extension.pipelines_agent]
...
module "hybrid_worker_group" {
  ...
  name                  = var.automation_account_resource != null && var.hybrid_worker_group_name == null ? toset(module.virtual_machine.name) : null
  automation_account_id = var.automation_account_resource == null ? null : var.automation_account_resource.id
  credential_name       = var.automation_account_credential_name
}

module "hybrid_worker" {
  source = "../hybrid_worker"

  automation_account_name     = var.automation_account_resource == null ? null : var.automation_account_resource.name
  automation_account_rg_name  = var.automation_account_resource == null ? null : var.automation_account_resource.resource_group_name
  virtual_machine_resource    = var.automation_account_resource == null ? null : local.virtual_machine_resource
  automation_account_endpoint = var.automation_account_resource == null ? null : var.automation_account_resource.endpoint

  module_depends_on = [
    module.hybrid_worker_group.name
  ]
}
