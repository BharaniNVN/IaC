    ...
    if($Ensure -eq 'Present')
    {
        ...
        if(!$WsusConfiguration.SyncFromMicrosoftUpdate)
        {
            ...
            Write-Verbose -Message "WSUSServer upstream server is $UpstreamServerName, `
                ...
                updates are downloaded from Microsfot Update $GetContentFromMU."
        }
        else
        {
            $UpstreamServerName = ""
...
<#
    ...
    Boolean to specify whether update binaries are downloaded from Microsoft Update or from the upstream server
...
function Test-TargetResource
{
    ...
    if($result -and ($Wsus.Ensure -eq "Present"))
    {
        ...
        if($PSBoundParameters.ContainsKey('UpstreamServerName'))
        {
            ...
            {
                Write-Verbose -Message "GetContentFromMU test failed"
                $result = $false
            }
        }
        # Test Proxy Server
        ...
        # Test Products
        if($PSBoundParameters.ContainsKey('Products') -and $null -ne (Compare-Object -ReferenceObject ($Wsus.Products | Sort-Object -Unique) `
        ...
        if($PSBoundParameters.ContainsKey('Classifications') -and $null -ne (Compare-Object -ReferenceObject ($Wsus.Classifications | Sort-Object -Unique) `
