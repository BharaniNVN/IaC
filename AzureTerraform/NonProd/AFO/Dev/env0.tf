...
module "afo_canary" {
  ...
  resource_group_resource                 = azurerm_resource_group.this
  resource_prefix                         = local.prefix
  virtual_machine_suffix                  = ["-afoaz03", "-afoaz04"]
  enable_availability_set                 = false
  availability_set_id                     = module.afo.availability_set_id
  enable_boot_diagnostics_storage_account = false
  boot_diagnostics_storage_blob_endpoint  = module.afo.boot_diagnostics_storage_blob_endpoint
  subnet_resource                         = azurerm_subnet.dmz
  dns_servers                             = data.terraform_remote_state.nonprod_shared.outputs.domain_specifics.dns_servers
  vm_starting_ip                          = var.afo_lb_ip + 3
  vm_size                                 = "Standard_F4s_v2"
  user_assigned_identity_ids              = [azurerm_user_assigned_identity.key_vault_certificates.id]
  certificate_urls = [
    data.terraform_remote_state.nonprod_shared.outputs.certificates["mxhhpdev_com"],
  ...
  firewall_ports = [80, 443, 8001]
  folders_permissions = {
    "IIS_IUSRS" = { "Read" = ["C:\\BT\\ver", "C:\\AMS\\AMSRoot", "C:\\Applications\\Documents"], "FullControl" = ["C:\\Logs", "C:\\Temp"] },
  }
  dns_records                             = []
  nxlog_conf                              = var.nxlog_conf
  nxlog_pem                               = var.nxlog_pem
  ...
  azure_devops_project                    = var.azure_devops_project
  azure_devops_deployment_group           = var.azure_devops_deployment_group
  azure_devops_agent_tags                 = "AFO Canary"
  azure_devops_pat_token                  = var.azure_devops_pat_token
  dependency_agent_extension_version      = var.dependency_agent_extension_version
  microsoft_antimalware_extension_version = var.microsoft_antimalware_extension_version
...
module "clinical_content" {
  source = "../../../../modules/terraform/afo_server"

  resource_group_resource                 = azurerm_resource_group.this
  resource_prefix                         = local.prefix
  virtual_machine_suffix                  = ["-clincont"]
  enable_availability_set                 = false
  availability_set_id                     = module.afo.availability_set_id
  enable_boot_diagnostics_storage_account = false
  boot_diagnostics_storage_blob_endpoint  = module.afo.boot_diagnostics_storage_blob_endpoint
  subnet_resource                         = azurerm_subnet.dmz
  dns_servers                             = data.terraform_remote_state.nonprod_shared.outputs.domain_specifics.dns_servers
  vm_starting_ip                          = var.afo_lb_ip + 8
  vm_size                                 = "Standard_F4s_v2"
  user_assigned_identity_ids              = [azurerm_user_assigned_identity.key_vault_certificates.id]
  certificate_urls = [
    data.terraform_remote_state.nonprod_shared.outputs.certificates["mxhhpdev_com"],
  ...
  azure_devops_project                    = var.azure_devops_project
  azure_devops_deployment_group           = var.azure_devops_deployment_group
  azure_devops_agent_tags                 = "Clinical Content"
  azure_devops_pat_token                  = var.azure_devops_pat_token
  dependency_agent_extension_version      = var.dependency_agent_extension_version
  microsoft_antimalware_extension_version = var.microsoft_antimalware_extension_version
...
module "app" {
  ...
  local_groups_members                    = { "Administrators" = formatlist("%s\\%s", local.domain_netbios_name, [var.hangfire_service_account, var.nondbservers_local_administrators]) }
...
module "oracledevsandbox" {
  source = "../../../../modules/terraform/oracle"

  resource_group_resource                 = azurerm_resource_group.this
  resource_prefix                         = local.prefix
  virtual_machine_suffix                  = ["-oradev"]
  availability_set_suffix                 = "-oraaz-av01"
  boot_diagnostics_storage_account_suffix = "oraazmxhhpdiagg"
  subnet_resource                         = azurerm_subnet.internal
  dns_servers                             = data.terraform_remote_state.nonprod_shared.outputs.domain_specifics.dns_servers
  vm_starting_ip                          = 9
  vm_size                                 = "Standard_E4s_v3"
