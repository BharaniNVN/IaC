...
resource "azurerm_monitor_diagnostic_setting" "analytics_int_sqldb" {
...
resource "azurerm_sql_firewall_rule" "offices_and_contractors_public_ips" {
  for_each = merge(data.terraform_remote_state.nonprod_shared.outputs.office_public_ip_address, { "RamitOffice" = "104.251.104.70" })
