...
resource "azurerm_key_vault_secret" "bt_datamart_connection_string" {
  ...
  value        = "Data Source=${azurerm_sql_server.analytics.fully_qualified_domain_name};Initial Catalog=${azurerm_sql_database.analytics.name};User id=${var.azure_sql_admin_user};Password=${var.azure_sql_admin_pswd};Encrypt=yes;trustServerCertificate=true"
  key_vault_id = azurerm_key_vault.this.id
...
resource "azurerm_key_vault_secret" "hhp_staging_db_connection_string" {
  ...
  value        = "Data Source=${lookup(module.sql.name_with_ip_address_and_port, "hfsqlaze2int01")};Initial Catalog=BtStaging;User id=${var.sql_login_adfuser_name};Password=${var.sql_login_adfuser_pswd};Encrypt=yes;trustServerCertificate=true"
  key_vault_id = azurerm_key_vault.this.id
