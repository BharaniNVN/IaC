...
resource "azurerm_key_vault_secret" "hhp_staging_db_connection_string" {
  ...
  value        = "Data Source=${lookup(module.sql.name_with_ip_address_and_port, "desqlaze2int01")};Initial Catalog=BtStaging;User id=${var.sql_login_adfuser_name};Password=${var.sql_login_adfuser_pswd};Encrypt=yes;trustServerCertificate=true"
  key_vault_id = azurerm_key_vault.this.id
