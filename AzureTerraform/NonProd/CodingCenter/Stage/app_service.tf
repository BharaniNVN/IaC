resource "azurerm_app_service" "this" {
  ...
  app_service_plan_id = data.terraform_remote_state.codingcenter_shared.outputs.app_service_plan_id
  https_only          = true
  ...
  app_settings = merge(
    data.external.app_service_appsettings.result,
    {
      ...
      "APPLICATIONINSIGHTS_CONNECTION_STRING"           = format("InstrumentationKey=%s", module.application_insights.instrumentation_key)
      "ApplicationInsightsAgent_EXTENSION_VERSION"      = "~2"
      "btServiceBusQueueName"                           = data.terraform_remote_state.oasis_dev.outputs.servicebus_queue_name
      "btServiceBusConnectionString"                    = data.terraform_remote_state.oasis_dev.outputs.servicebus_queue_auth_rule_name
...
resource "azurerm_app_service_virtual_network_swift_connection" "app_service" {
  ...
  subnet_id      = data.terraform_remote_state.codingcenter_shared.outputs.app_service_subnet.id
}
