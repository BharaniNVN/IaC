...
resource "azurerm_mssql_firewall_rule" "office_public_ips" {
  ...
  name             = format("%s-sql-fw-%s", local.prefix, each.key)
  server_id        = azurerm_mssql_server.this.id
...
resource "azurerm_mssql_firewall_rule" "app_service_plan" {
  name             = "${local.prefix}-sql-fw-asp"
  server_id        = azurerm_mssql_server.this.id
  start_ip_address = cidrhost(data.terraform_remote_state.oasis_shared.outputs.app_service_subnet.address_prefixes[0], 0)
  end_ip_address   = cidrhost(data.terraform_remote_state.oasis_shared.outputs.app_service_subnet.address_prefixes[0], -1)
}
