...
resource "azurerm_servicebus_subscription" "hfxafo" {
  name               = "hfxafo"
  topic_id           = azurerm_servicebus_topic.recommendations.id
  max_delivery_count = 10
}

resource "azurerm_servicebus_subscription_rule" "hfxafo" {
  name            = "CorrelationRule"
  subscription_id = azurerm_servicebus_subscription.hfxafo.id
  filter_type     = "CorrelationFilter"

  correlation_filter {
    correlation_id = "hfxafo"
  }
}
