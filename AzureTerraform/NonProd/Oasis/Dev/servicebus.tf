...
resource "azurerm_servicebus_subscription" "t6" {
  name               = "T6"
  topic_id           = azurerm_servicebus_topic.recommendations.id
  max_delivery_count = 10
}

resource "azurerm_servicebus_subscription_rule" "t6" {
  name            = "CorrelationRule"
  subscription_id = azurerm_servicebus_subscription.t6.id
...
resource "azurerm_servicebus_subscription" "devafo" {
  name               = "devafo"
  topic_id           = azurerm_servicebus_topic.recommendations.id
  max_delivery_count = 10
}

resource "azurerm_servicebus_subscription_rule" "devafo" {
  name            = "CorrelationRule"
  subscription_id = azurerm_servicebus_subscription.devafo.id
  filter_type     = "CorrelationFilter"

  correlation_filter {
    correlation_id = "devafo"
  }
}
