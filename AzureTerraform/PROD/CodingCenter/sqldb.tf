resource "azurerm_mssql_server" "this" {
  ...
  azuread_administrator {
    login_username = data.terraform_remote_state.prod_shared.outputs.sql_admins_group.display_name
...
resource "azurerm_mssql_firewall_rule" "app_service_plan" {
  ...
  server_id        = azurerm_mssql_server.this.id
  start_ip_address = cidrhost(azurerm_subnet.app_service.address_prefixes[0], 0)
  end_ip_address   = cidrhost(azurerm_subnet.app_service.address_prefixes[0], -1)
}
