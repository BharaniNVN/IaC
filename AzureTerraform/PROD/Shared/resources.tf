...
resource "azurerm_key_vault" "prod_shared_old" {
  name                            = "${local.deprecated_prefix}shared-kv-bthhh"
  location                        = azurerm_resource_group.resources.location
  resource_group_name             = azurerm_resource_group.resources.name
...
resource "azurerm_monitor_diagnostic_setting" "key_vault_old" {
  name                       = "SendAllToLogAnalytics"
  target_resource_id         = azurerm_key_vault.prod_shared_old.id
...
resource "azurerm_eventhub_namespace" "alienvault" {
  name                = "${local.deprecated_prefix}-eventhubnamespace-alienvault"
  location            = azurerm_resource_group.resources.location
  resource_group_name = azurerm_resource_group.resources.name
  sku                 = "Basic"
  capacity            = 1
...
resource "azurerm_eventhub" "alienvault" {
  name                = "${local.deprecated_prefix}-eventhub-alienvault"
...
resource "azurerm_storage_account" "ops" {
  name                            = "${local.prefix}opsazmxhhpsa"
  resource_group_name             = azurerm_resource_group.resources.name
  location                        = azurerm_resource_group.resources.location
  account_tier                    = "Standard"
  ...
  network_rules {
    default_action             = "Deny"
    ip_rules                   = concat(values(var.office_public_ip_address), flatten([for _, v in data.dns_a_record_set.nonprod_fw_pips : v.addrs]))
    bypass                     = ["Logging"]
    virtual_network_subnet_ids = [data.terraform_remote_state.shared.outputs.azure_pipelines_agent_subnet_id]
  }
...
resource "azurerm_storage_share" "ops" {
  name                 = "deployments"
