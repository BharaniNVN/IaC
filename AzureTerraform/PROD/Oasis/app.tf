resource "azurerm_windows_web_app" "this" {
  name                    = "${local.deprecated_application_prefix}-as"
...
resource "azuread_application" "api" {
  ...
  identifier_uris  = [format("https://%s/%s", data.terraform_remote_state.prod_shared.outputs.aad_domain_name, local.application_name_api)]
  owners           = [data.azurerm_client_config.current.object_id]
  sign_in_audience = "AzureADMultipleOrgs"
