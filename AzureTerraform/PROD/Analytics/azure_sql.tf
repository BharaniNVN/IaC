resource "azurerm_sql_server" "analytics" {
  ...
  administrator_login_password = var.sql_sa_pswd
...
module "az_sql_audit" {
  ...
  resource_group_name       = azurerm_resource_group.analytics.name
  log_analytics_resource_id = data.terraform_remote_state.prod_shared.outputs.log_analytics.id
  eventhub_policy_id        = data.terraform_remote_state.prod_shared.outputs.eventhub_policy.id
...
resource "azurerm_monitor_diagnostic_setting" "analytics_sqldb" {
...
resource "azurerm_sql_active_directory_administrator" "ad_admin" {
  server_name         = azurerm_sql_server.analytics.name
  resource_group_name = azurerm_resource_group.analytics.name
  login               = data.terraform_remote_state.prod_shared.outputs.sql_admins_group.display_name
  tenant_id           = data.azurerm_client_config.current.tenant_id
  object_id           = data.terraform_remote_state.prod_shared.outputs.sql_admins_group.object_id
}
...
resource "azurerm_sql_firewall_rule" "offices_public_ips" {
  for_each = data.terraform_remote_state.prod_shared.outputs.office_public_ip_address
