resource "azurerm_resource_group" "env4" {
  name     = "${local.prefix}-env4-rg"
  location = var.location

  tags = merge(
    local.tags,
    {
      "logicalEnvironment" = "DR4"
...
module "afo4" {
  ...
  resource_group_resource                 = azurerm_resource_group.env4
  resource_prefix                         = local.prefix
  virtual_machine_suffix                  = ["-afoaz41", "-afoaz42"]
  availability_set_suffix                 = "-afoaz4-av"
  boot_diagnostics_storage_account_suffix = "afoaz4mxhhpdiag"
  subnet_resource                         = local.dmz_subnet
  dns_servers                             = local.dmz_domain["dns_servers"]
  vm_starting_ip                          = var.afo4_vm_starting_ip
  ...
  azure_devops_agent_tags                 = "AFO, DR4"
...
module "app4" {
  ...
  resource_group_resource                 = azurerm_resource_group.env4
  resource_prefix                         = local.prefix
  virtual_machine_suffix                  = ["-appaz41"]
  availability_set_suffix                 = "-appaz4-av"
  boot_diagnostics_storage_account_suffix = "appaz4mxhhpdiag"
  ...
  azure_devops_project                    = var.azure_devops_project
  azure_devops_deployment_group           = "US-Azure-NorthCentral-US-DR"
  azure_devops_agent_tags                 = "APP, DR4"
  azure_devops_pat_token                  = var.azure_devops_pat_token
  dependency_agent_extension_version      = var.dependency_agent_extension_version
  microsoft_antimalware_extension_version = var.microsoft_antimalware_extension_version
...
module "sql4" {
  ...
  resource_group_resource                 = azurerm_resource_group.env4
  resource_prefix                         = local.prefix
  virtual_machine_suffix                  = ["-sqlaz41"]
  availability_set_suffix                 = "-sqlaz4-av"
  boot_diagnostics_storage_account_suffix = "sqlaz4mxhhpdiag"
  subnet_resource                         = local.internal_subnet
  dns_servers                             = local.internal_domain["dns_servers"]
  vm_starting_ip                          = 72
  vm_size                                 = "Standard_E32s_v3"
  data_disk = [
    { "name" = "db", "type" = "Premium_LRS", "size" = 3300, "lun" = 0, "caching" = "ReadOnly" },
  ...
  azure_devops_project                    = var.azure_devops_project
  azure_devops_deployment_group           = "US-Azure-NorthCentral-US-DR"
  azure_devops_agent_tags                 = "DB, MsSql, Clinical, DR4"
  azure_devops_pat_token                  = var.azure_devops_pat_token
  dependency_agent_extension_version      = var.dependency_agent_extension_version
  microsoft_antimalware_extension_version = var.microsoft_antimalware_extension_version
...
module "oracle4" {
  ...
  resource_group_resource                 = azurerm_resource_group.env4
  resource_prefix                         = local.prefix
  virtual_machine_suffix                  = ["-oraaz41"]
  availability_set_suffix                 = "-oraaz4-av"
  boot_diagnostics_storage_account_suffix = "oraaz4mxhhpdiag"
  subnet_resource                         = local.internal_subnet
  dns_servers                             = local.internal_domain["dns_servers"]
  vm_starting_ip                          = 92
  vm_size                                 = "Standard_E32s_v3"
  ...
  azure_devops_project                    = var.azure_devops_project
  azure_devops_deployment_group           = "US-Azure-NorthCentral-US-DR"
  azure_devops_agent_tags                 = "DB, Oracle, DR4"
  azure_devops_pat_token                  = var.azure_devops_pat_token
  dependency_agent_extension_version      = var.dependency_agent_extension_version
  microsoft_antimalware_extension_version = var.microsoft_antimalware_extension_version
